...
// to the chart. NormalizedAvg is the load-bearing field (it's what the
...
type MuscleGroupProgressionPoint struct {
	...
	// NormalizedAvg = avg_estimated_1rm / current baseline. 1.0 means
	// the lifter performed at their current baseline capability on this
	// exercise; >1.0 above, <1.0 below. This is what gets plotted.
	NormalizedAvg float64 `json:"normalized_avg"`
...
//     normalized_avg = avg_estimated_1rm / baseline.
//
	...
	for _, h := range histories {
		...
		for _, e := range h.Entries {
			...
			result.Points = append(result.Points, MuscleGroupProgressionPoint{
				...
				NormalizedAvg:   round3(e.AvgEstimated1RM / baseline),
...
// NormalizedAvg field across every point, regardless of which exercise
